...
class c_verifikasi_kph extends Controller
{
    function list_verifikasi_kph(){
        ...
        ->where('verifikasi_kph','=',0)
        ->where('pemilahan_admin','=',1)
        ->get();
        $data['rhl'] = tb_rhl::join('tb_status_rhls','tb_status_rhls.rhl_id','=','tb_rhls.id_rhl')
        ->where('verifikasi_kph','=',0)
        ->where('pemilahan_admin','=',1)
        ->get();
        return view('kehutanan.admin.list_verifikasi_kph',$data);
    }

    function verifikasi_kph($id){
        ...
        return view('kehutanan.admin.verifikasi_kph',$data);
    }

    function verifikasi_rhl_kph($id){
        ...
        return view('kehutanan.admin.verifikasi_rhl_kph',$data);
    }

    

    function doverif_kph($id){
        $data = tb_verifikasi_persos::where('perhutanan_sosial_id','=',$id)->first();
        $data->verifikasi_kph = 1;
        if ($data->save()) {
            Session::flash('status','sukses');
            Session::flash('pesan','Verifikasi pada tahap KPH berhasil disimpan');
        } else {
            Session::flash('status','gagal');
            Session::flash('pesan','Verifikasi pada tahap KPH gagal disimpan');
        }
        return redirect('list_verifikasi_kph');
    }

    function doverif_rhl_kph($id){
